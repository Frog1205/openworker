...
export function SkillsTab({
  ...
  const fail = (res: { ok?: boolean; error?: string }) => {
    setNotice(null);
    if (res.ok === false) {
      setError(res.error || t("skills.error"));
      return true;
    }
    setError("");
    return false;
  ...
  return (
    <section>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <h2 className="text-[16px] font-semibold">{t("settings.skills")}</h2>
          ...
          {addOpen ? (
            <>
              ...
              >
                ...
                >
                  <div className="text-[13px] font-medium">{t("skills.write")}</div>
                  <div className="text-[11.5px] text-muted">{t("skills.writeHelp")}</div>
                </button>
                <button
                  role="menuitem"
                  className="w-full text-left px-3 py-2 rounded-lg hover:bg-paper"
                ...
                >
                  <div className="text-[13px] font-medium">{t("skills.import")}</div>
                  <div className="text-[11.5px] text-muted">{t("skills.importHelp")}</div>
                </button>
                <button
                  role="menuitem"
                ...
                >
                  <div className="text-[13px] font-medium">{t("skills.createWithAtlas")}</div>
                  <div className="text-[11.5px] text-muted">{t("skills.createWithAtlasHelp")}</div>
                </button>
              </div>
            </>
          ) : null}
      ...
      {upload ? (
        <div className={`${CARD} p-4 mb-4`}>
          <div className="text-[13px] font-medium mb-1">{t("skills.review")}</div>
          <p className="text-[12.5px] text-muted mb-3">{t("skills.reviewHelp")}</p>
          ...
          {upload.files?.length ? (
            <div className="text-[12px] text-muted mb-2">
              {t("skills.bundledFiles", { files: upload.files.join(", ") })}
            </div>
          ) : null}
      ...
      {editor ? (
        <div className={`${CARD} p-4 mb-4`}>
          <div className="text-[13px] font-medium mb-3">
            {editor.mode === "new" ? t("skills.new") : t("skills.editName", { name: editor.name })}
          </div>
      ...
      <div className={`${CARD} divide-y divide-line`}>
        {rows.length === 0 && !editor ? (
          <div className="p-5 text-[13px] text-muted">{t("skills.empty")}</div>
        ) : null}
        {rows.map((row) => (
          <div key={row.name} className="flex items-center gap-3 px-4 py-3">
            ...
            <button
              className={BTN_BORDERED}
              aria-label={t("skills.delete", { name: row.name })}
              onClick={() => remove(row)}
            ...
            <label className="inline-flex items-center gap-1.5 text-[12px] text-muted">
              <input
                type="checkbox"
                role="switch"
                aria-label={t("skills.enabled", { name: row.name })}
                checked={row.enabled}
                onChange={(e) => {
