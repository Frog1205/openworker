...
export function ModelsTab() {
  ...
  if (!settings) return <div className="text-[13px] text-muted">{t("settings.models.loading")}</div>;
  ...
  if (ps.sel === null) {
    return (
      <div>
        ...
        <CustomModelCard settings={settings} onChanged={refreshSettings} />
    ...
    <div>
        ...
        footer={
          ps.credentialed ? (
            <button
              ...
              onClick={() => {
                if (window.confirm(t("settings.models.removeKeyConfirm", { provider: info?.title || "" }))) ps.removeKey();
              }}
            >
              {t("settings.models.removeKey")}
            </button>
          ) : null
        }
      />
      ...
      {ps.sel === "openai" && settings.source === "env" && (
        <p className="text-[12px] text-muted mt-3 leading-relaxed">
          {t("settings.models.envKey")}
        </p>
      )}

      {info?.configured ? (
        <div className="mt-6">
          <div className={SEC_H + " mb-1.5"}>{t("settings.models.available")}</div>
          <p className="text-[12px] text-muted mb-2.5 leading-relaxed">{t("settings.models.availableHelp")}</p>
          <ModelChecklist
            provider={ps.sel}
        ...
        (info?.suggested_models?.length || 0) > 0 && (
          <div className="mt-6" data-testid="model-preview">
            <div className={SEC_H + " mb-1.5"}>{t("settings.models.included")}</div>
            <p className="text-[12px] text-muted mb-2.5 leading-relaxed">{t("settings.models.includedHelp")}</p>
...
function CustomModelCard({ settings, onChanged }: { settings: ModelSettings; onChanged: () => void }) {
  const { t } = useI18n();
  ...
  return <div className={`${CARD} mt-6 p-4`} data-testid="custom-model-card">
    ...
    {open && <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <label className="grid gap-1 text-[12px] text-muted"><span>{t("settings.models.custom.name")}</span><input className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-[13px]" placeholder="例如：GPTGod DeepSeek" value={name} onChange={(e) => setName(e.target.value)} /></label>
      <label className="grid gap-1 text-[12px] text-muted"><span>{t("settings.models.custom.vendor")}</span><input className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-[13px]" placeholder="例如：GPTGod" value={vendor} onChange={(e) => setVendor(e.target.value)} /></label>
      <label className="grid gap-1 text-[12px] text-muted"><span>{t("settings.models.custom.model")}</span><input className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-[13px]" placeholder="例如：deepseek-chat（不是接口地址）" value={model} onChange={(e) => setModel(e.target.value)} /></label>
      <label className="grid gap-1 text-[12px] text-muted"><span>{t("settings.models.custom.protocol")}</span><select className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-[13px]" value={protocol} onChange={(e) => setProtocol(e.target.value as "openai" | "claude")}><option value="openai">OpenAI 协议（Chat Completions）</option><option value="claude">Claude 协议（Messages）</option></select></label>
      <label className="grid gap-1 text-[12px] text-muted"><span>{t("settings.models.custom.endpoint")}</span><input className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-[13px]" placeholder="https://api.gptgod.online/v1" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} /></label>
...
function ComposerPickerCard({
  ...
  return (
    <div className="mt-6" data-testid="composer-picker">
      <div className={SEC_H + " mb-1.5"}>{t("settings.models.picker")}</div>
      <p className="text-[12px] text-muted mb-2.5 leading-relaxed">{t("settings.models.pickerHelp")}</p>
      <div className="mlist">
            ...
            <div className="mlist-row" key={id}>
              <label className="mlist-main">
                <input
                  ...
                  checked
                  disabled={isDefault}
                  title={isDefault ? t("settings.models.defaultLocked") : t("settings.models.removeFromPicker")}
